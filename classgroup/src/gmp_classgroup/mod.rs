...
impl GmpClassGroup {
  ...
  fn inner_multiply(&mut self, rhs: &Self, ctx: &mut Ctx) {
    ...
    ctx.congruence_context.solve_linear_congruence(&mut ctx.mu, Some(&mut ctx.v), &ctx.a, &ctx.b, &ctx.m);
    ...
    ctx.congruence_context.solve_linear_congruence(&mut ctx.lambda, Some(&mut ctx.sigma), &ctx.a, &ctx.b, &ctx.m);
  ...
  fn new(a: Mpz, b: Mpz, c: Mpz, discriminant: Mpz) -> Self {
    let s = GmpClassGroup { a, b, c, discriminant };
    s.assert_valid();
    s
  }
  ...
  fn inner_reduce(&mut self, ctx: &mut Ctx) {
    ...
    while if ffi::mpz_is_negative(&self.b) { self.a >= self.c } else { self.a > self.c } {
  ...
  fn inner_square_impl(&mut self, ctx: &mut Ctx) {
    self.assert_valid();
    ctx.congruence_context.solve_linear_congruence(&mut ctx.mu, None, &self.b, &self.c, &self.a);
...
impl ClassGroup for GmpClassGroup {
  ...
  fn deserialize(buf: &[u8], discriminant: Self::BigNum) -> Self {
    ...
    let half_len = len >> 1;
    Self::from_ab_discriminant(ffi::import_obj(&buf[..half_len]), ffi::import_obj(&buf[half_len..]), discriminant)
  }
...
mod test {
  ...
  #[test]
  fn normalize() {
    let mut s = GmpClassGroup::new(16.into(), (-23).into(), 5837_3892.into(), (-0xdead_beefi64).into());
