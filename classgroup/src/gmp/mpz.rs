...
extern "C" {
  ...
  fn __gmpz_import(rop: mpz_ptr, count: size_t, order: c_int, size: size_t, endian: c_int, nails: size_t, op: *const c_void);
  fn __gmpz_export(rop: *mut c_void, countp: *mut size_t, order: c_int, size: size_t, endian: c_int, nails: size_t, op: mpz_srcptr);
...
impl<'a> From<&'a [u8]> for Mpz {
  fn from(other: &'a [u8]) -> Mpz {
    unsafe {
      let mut res = Mpz::new();
      __gmpz_import(&mut res.mpz, other.len(), 1, size_of::<u8>() as size_t, 0, 0, other.as_ptr() as *const c_void);
      res
    }
  }
}
...
impl From<u64> for Mpz {
  fn from(other: u64) -> Mpz {
    unsafe {
      let mut res = Mpz::new();
      __gmpz_import(&mut res.mpz, 1, -1, size_of::<u64>() as size_t, 0, 0, &other as *const u64 as *const c_void);
      res
    }
  }
}
...
impl From<u32> for Mpz {
  fn from(other: u32) -> Mpz {
    unsafe {
      let mut res = Mpz::new();
      __gmpz_import(&mut res.mpz, 1, -1, size_of::<u32>() as size_t, 0, 0, &other as *const u32 as *const c_void);
      res
    }
  }
}
  ...
  fn from(other: i64) -> Mpz {
    unsafe {
      ...
      if other.is_negative() {
        __gmpz_import(&mut res.mpz, 1, -1, size_of::<i64>() as size_t, 0, 0, &(other ^ -1i64) as *const i64 as *const c_void);
        __gmpz_com(&mut res.mpz, &res.mpz);
      } else {
        __gmpz_import(&mut res.mpz, 1, -1, size_of::<i64>() as size_t, 0, 0, &other as *const i64 as *const c_void);
      }
      res
    }
  }
...
impl From<i32> for Mpz {
  fn from(other: i32) -> Mpz {
    unsafe {
      ...
      if other.is_negative() {
        __gmpz_import(&mut res.mpz, 1, -1, size_of::<i32>() as size_t, 0, 0, &(other ^ -1i32) as *const i32 as *const c_void);
        __gmpz_com(&mut res.mpz, &res.mpz);
      } else {
        __gmpz_import(&mut res.mpz, 1, -1, size_of::<i32>() as size_t, 0, 0, &other as *const i32 as *const c_void);
      }
      res
    }
  }
