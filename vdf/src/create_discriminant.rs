...
/// The discriminant is guaranteed to be a negative prime number that fits in `length` bits, except with negligible probability (less than 2^(-100)).  
/// It is also guaranteed to equal 7 modulo 8.
///
/// This function uses sha256 to expand the seed.  
/// Therefore, different seeds will result in completely different discriminants with overwhelming
